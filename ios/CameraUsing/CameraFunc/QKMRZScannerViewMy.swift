...
public protocol QKMRZScannerViewDelegateMy: AnyObject {
    func mrzScannerView(_ mrzScannerView: QKMRZScannerViewMy, didFind scanResult: QKMRZScanResultMy)
}

@IBDesignable
public class QKMRZScannerViewMy: UIView {
    fileprivate let tesseract = SwiftyTesseract(language: .custom("ocrb"), dataSource: Bundle(for: QKMRZScannerViewMy.self), engineMode: .tesseractOnly)
    ...
    fileprivate let cutoutView = QKCutoutViewMy()
    fileprivate var isScanningPaused = false
    fileprivate var observer: NSKeyValueObservation?
    @objc public dynamic var isScanning = false
    public var vibrateOnResult = true
    public weak var delegate: QKMRZScannerViewDelegateMy?
    ...
    // MARK: MRZ
    fileprivate func mrz(from cgImage: CGImage) -> QKMRZScanResultMy? {
    ...
    public func scanId(from cgImage: CGImage) -> CGImage {
        let croppingRect = cutoutRect(for: cgImage)
        return cgImage.cropping(to: croppingRect) ?? cgImage
    }
    ...
    fileprivate func initCaptureSession() {
      ...
      guard var camera = AVCaptureDevice.default(.builtInWideAngleCamera, for: .video, position: .back) else {
...
extension QKMRZScannerViewMy: AVCaptureVideoDataOutputSampleBufferDelegate {
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        let detectTextRectangles = VNDetectTextRectanglesRequest { [unowned self] request, error in
          ...
          if pressScan.scanButton != 0 {
            self.stopScanning()
            
          DispatchQueue.main.async {
              let enlargedDocumentImage = self.enlargedDocumentImage(from: cgImage)
              let scanResult = QKMRZScanResultMy(documentImage: enlargedDocumentImage)
